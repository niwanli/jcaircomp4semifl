...
save_address='performance_vs_rounds\SemiFL_with_sampling_and_pruning';
...
%     global_net_old_vector=[vec(global_net.IW{1,1});vec(global_net.LW{2,1});vec(global_net.b{1});vec(global_net.b{2})];
...
%     global_net.LW{2,1}=reshape(global_net_new_vector(start_point+1:start_point+size(vec(global_net.LW{2,1}),1)),size(global_net.LW{2,1}));
%     start_point=start_point+size(vec(global_net.LW{2,1}),1);
%     global_net.b{1}=reshape(global_net_new_vector(start_point+1:start_point+size(vec(global_net.b{1}),1)),size(global_net.b{1}));
%     start_point=start_point+size(vec(global_net.b{1}),1);
%     global_net.b{2}=reshape(global_net_new_vector(start_point+1:start_point+size(vec(global_net.b{2}),1)),size(global_net.b{2}));
